...
import { ArrowLeft, Download, FileArchive, FileText, Plus } from "lucide-react";
...
export default function ReportDetailPage() {
  ...
  return (
    <>
      <Link href="/reports/history" className="mb-4 inline-flex items-center gap-2 text-sm text-muted hover:text-foreground"><ArrowLeft className="size-4" />К истории</Link>
      ...
      {report.status === "completed" ? (
        <div className="space-y-7">
          <section className="border-y border-green-200 bg-green-50 px-4 py-4 dark:border-green-900 dark:bg-green-950/25">
            ...
            <p className="mt-1 text-sm text-green-800 dark:text-green-300">Файлы сохранены в истории и готовы к скачиванию.</p>
          </section>
          ...
          <section>
            <div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">Файлы</h2>{report.files.length > 1 ? <Button asChild variant="secondary"><a href={`/api/reports/${report.id}/files.zip`}><FileArchive className="size-4" />Скачать ZIP</a></Button> : null}</div>
