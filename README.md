...
- `backend`: FastAPI, автономный PaddleOCR с русской PP-OCRv5-моделью, Poppler и ZBar;
- `db`: PostgreSQL 17;
...
PaddleOCR и CPU runtime устанавливаются из Python-зависимостей, а модели находятся в `vendor/paddleocr`, поэтому при распознавании интернет не нужен. Для резервного рендеринга PDF локально требуется Poppler; в Docker он уже включён.
